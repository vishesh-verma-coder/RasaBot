...
document.addEventListener('DOMContentLoaded', function() {
    ...
    instancesTap.open();
    setTimeout(function() { instancesTap.close(); }, 8000);
...
function action_trigger() {
    ...
    $.ajax({
        url: `https://iyantraesc.online/conversations/${user_id}/execute`,
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify({ "name": action_name, "policy": "MappingPolicy", "confidence": "0.98" }),
		
		//data: JSON.stringify({ message: 'Hello', sender: user_id }),
	    //data: JSON.stringify({ "name": action_name,"policy":"policy_1_TEDPolicy" }),
		//data: JSON.stringify({ message: "Hello", sender: user_id }),
...
function send(message) {
    ...
    $.ajax({
        url: "https://iyantraesc.online/webhooks/rest/webhook",
        type: "POST",
        contentType: "application/json",
...
function setBotResponse(response) {
    ...
    setTimeout(function() {
        ...
        if (response.length < 1) {
			...
			if (message=="get_started_test")
			{
				var fallbackMsg = "Successfull";
			}
			
            var fallbackMsg = "I am facing some issues, please report to ESC Team!!!";

            var BotResponse = '<img class="botAvatar" src="./static/img/robot_hello.gif"/><p class="botMsg">' + fallbackMsg + '</p><div class="clearfix"></div>';
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("text")) {
                    var BotResponse = '<img class="botAvatar" src="./static/img/robot_hello.gif"/><p class="botMsg">' + response[i].text + '</p><div class="clearfix"></div>';
                    $(BotResponse).appendTo(".chats").hide().fadeIn(1000);
                }
...
function showBotTyping() {
    ...
    var botTyping = '<img class="botAvatar" id="botAvatar" src="./static/img/robot_hello.gif"/><div class="botTyping">' + '<div class="bounce1"></div>' + '<div class="bounce2"></div>' + '<div class="bounce3"></div>' + '</div>'
